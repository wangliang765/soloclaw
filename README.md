...
- Policy-gated workspace tools, file-level write locks, approval replay/resume, signed audit bundle export, and local execution hygiene checks.
...
2. Long-running local coding agent for real repositories.
   ...
   - Current state: partially implemented; unified-diff `apply_patch` now has workspace-boundary validation, file locks, file-change records, command audit, command timeout/duration evidence, local command execution profile metadata, `agent run --json --session-result --verify-session` for end-to-end session evidence, `agent resume --json --session-result --verify-session` for resumed-session evidence, `agent plan|build|goal --json --session-result --verify-session` for target-mode evidence, `agent session diff <session-id>` for persisted patch inspection, `agent session report <session-id> --json` for consolidated engineering evidence including approvals, timed-out commands, and command execution profiles, `agent session status <session-id>` for daemon-ready status snapshots, `agent session timeline|logs <session-id>` for ordered audit/file-change/approval logs, `agent session review <session-id>` for a single operator review package across checklist, changes, commands, approvals, and latest timeline, `agent session result <session-id>` for a human-readable outcome/recovery/change/approval/timeout/profile summary, `agent session verify <session-id>` for machine-checkable engineering gates including required execution profiles, and `agent phase2 verify --json --cleanup` for local failed-test recovery, mock agent-loop repair, workspace-write/dependency/Git/high-risk-shell approval boundaries, timeout/profile/timeline/status/review evidence, diff/report/result/verification/run-session/resume/target-mode evidence, and pause/resume/cancel lifecycle smoke. The remaining gaps are stronger execution isolation beyond local policy/audit profiles, daemon service UX, smoother real-model task driving, richer generated diff/result inspection, and better Web/TUI inspection.
...
- Phase 2: Engineering execution capability.
  ...
  - Current partial smoke: `agent phase2 verify --json --cleanup` creates disposable sample repos, observes a failing test, records a timed-out command with local execution profile metadata, applies a unified diff patch, reruns the test, verifies persisted `session diff` / `session report` / `session status` / `session timeline` / `session review` / `session result` / `session verify` evidence, exercises end-to-end `agent run`, `agent resume`, and plan/build/goal target-mode session evidence paths, drives a mock agent-loop repair through real `run_command` / `apply_patch` / `run_command` tools, proves workspace-write/dependency-install/Git-mutation/high-risk-shell policy approval boundaries, runs pause/resume/cancel lifecycle smoke, and returns session-scoped file-change/audit/approval/timeout/profile/timeline/status/review evidence with `phaseClosure: "partial"`.
  - Current diff view: `agent session diff <session-id> [--json]` extracts successfully completed `apply_patch` unified diffs from session audit, maps them to changed paths, and prints the applied patch text for review.
  - Current evidence view: `agent session report <session-id> --json` summarizes messages, tool results, file changes, changed paths, command audit, command durations/timeouts, command execution profiles, approvals, failed tools, and failed commands for a persisted session.
  - Current live-status view: `agent session status <session-id> [--json] [--limit n]` returns outcome, command, approval, execution-profile, and latest timeline snapshots for daemon/TUI/Web reuse.
  ...
  - Current review view: `agent session review <session-id> [--json] [--limit n]` returns one operator package with review state, checklist, changed paths, patch excerpts, command results, recovery, approvals, latest safe timeline, and follow-up commands.
  - Current result view: `agent session result <session-id> [--json]` summarizes task outcome, failed-command recovery, changed files, patch count, command results, command durations/timeouts, command execution profiles, approvals, and review commands for a persisted engineering session.
  - Current verification gate: `agent session verify <session-id> [--json] [--require-change] [--require-patch] [--require-recovery] [--require-timeout] [--require-execution-profile profile] [--require-approval-action action]` exits non-zero when required engineering, timeout, execution-profile, or approval-action evidence is missing.
