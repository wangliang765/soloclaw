...
- Phase 2: Engineering execution capability.
  ...
  - Current diff view: `agent session diff <session-id> [--json]` extracts successfully completed `apply_patch` unified diffs from session audit, maps them to changed paths, reports per-file additions/deletions plus change type, patch count, review size, review hint, aggregate review profile, and prints the applied patch text for review.
