...
TUI `/approvals`, `/approve`, and `/deny` use the same approval store, scoped capability checks, `tool.approved` / `tool.denied` decision audit records, and room approval messages as the CLI approval path; replay/resume continuation remains explicit through `agent approve --auto-replay`, `--auto-resume`, or `--queue-resume`, and `agent phase2 verify --json --cleanup` now exercises the TUI list/approve/deny path as part of the local engineering smoke.
