...
TUI `/approvals`, `/approve`, and `/deny` use the same approval store, scoped capability checks, decision audit records, and room approval messages as the CLI approval path; replay/resume continuation remains explicit through `agent approve --auto-replay`, `--auto-resume`, or `--queue-resume`.
