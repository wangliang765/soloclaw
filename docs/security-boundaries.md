...
TUI `/session status` and `/session timeline|logs` are interactive aliases for the same safe session status and timeline projections; they do not replay raw tool input, prompt text, response text, or secret material.
