...
Current Phase 2 implementation note: workspace command policy classifies `run_command` calls into safe shell, dependency install, raw Git mutation, and high-risk shell actions. Dependency installs, raw Git mutations, high-risk shell commands, and balanced-mode workspace writes require approval, are audited as policy/tool events, are visible in `agent session report` / `agent session inspect` / `agent session next` / `agent session review` / `agent session result` / `agent session bundle`, and are exercised by `agent phase2 verify --json --cleanup`. Approved session-scoped tool calls can be replayed and either resumed synchronously with `agent approve --auto-replay --auto-resume` or queued for a local worker lease with `agent approve --queue-resume <worker-id>`, so approval continuation evidence is visible through the same task assignment and session audit paths. The Phase 2 verifier also exercises a local queued-approval retry path: an approved resume assignment expires, recovery schedules a retry with deterministic `retryDelayMs` / `retryNotBefore` metadata, workers skip it before it is due, operator queue and assignment views show `retry_delayed`, and the retry completes after the due time is advanced. Workspace command finishes also record duration, timeout, and local execution profile metadata (`local-safe`, `local-workspace-write`, `local-network`, or `local-full-access`) so bounded execution failures and declared execution boundaries are visible in the same session evidence path. `agent run|ask|plan|build|goal --require-model-ready` and `agent resume --require-model-ready` check real-model provider/base URL/API-key env or secret-ref readiness before opening or continuing a session; blocked output is metadata-only and `--api-key-secret` is reported only as configured. `agent session report`, token-gated `GET /api/sessions/:sessionId/report`, `agent session status`, `agent session inspect`, token-gated `GET /api/sessions/:sessionId/inspect`, `agent session next`, token-gated `GET /api/sessions/:sessionId/next`, `agent session review`, token-gated `GET /api/sessions/:sessionId/review`, `agent session result`, and `agent session bundle` aggregate session-scoped `model.called` counts, failures, provider-reported token usage, and duration from safe audit metadata without exposing prompt text, response text, tool inputs, or secret material; `agent session verify --require-model-call` can make a successful model call part of the machine gate, and `agent session verify --require-no-pending-approvals` can fail a handoff while approval requests remain unresolved. `agent session diff`, `agent session report`, token-gated `GET /api/sessions/:sessionId/report`, `agent session status`, `agent session inspect`, TUI `/session inspect`, `GET /api/sessions/:sessionId/inspect`, `agent session next`, TUI `/session next`, `GET /api/sessions/:sessionId/next`, `agent session review`, `GET /api/sessions/:sessionId/review`, `agent session result`, `agent session bundle`, `agent session verify --require-diff-stat`, and `agent session verify --require-review-profile` expose machine-readable file/addition/deletion diff stats plus per-file change type, patch count, review size, review hint, aggregate review profile, and inspection issue/focus-path metadata from successfully completed patch/session audit; `agent session status`, `agent session review`, `agent session result`, `agent session next`, and `agent session bundle` also expose handoff state/next-command metadata and operator next-action metadata. `agent sessions`, TUI `/sessions`, TUI `/sessions watch`, `agent local status`, `agent local service`, `agent local logs`, `soloclaw agent status`, `soloclaw agent service`, `soloclaw agent logs`, TUI `/agent status`, TUI `/agent service`, TUI `/agent logs`, TUI `/session inspect`, TUI `/session next`, TUI `/session watch`, `agent session status`, `agent session inspect`, `agent session next`, local control-plane session report, local control-plane session inspection, local control-plane session next, local control-plane session review, `agent session timeline|logs`, `agent session review`, and `agent session bundle` expose daemon-ready dashboard/status/log/review views from persisted audit, file-change, worker, assignment, and approval evidence using safe metadata instead of replaying raw tool inputs except bounded patch excerpts for explicit review. `agent workers poll` and `agent scheduler run` expose foreground daemon-loop lifecycle snapshots and aggregate metrics, and the phase2 verifier records a scheduler-run lifecycle smoke so local daemon UX can be checked without adding an unattended service. `agent session bundle` embeds local status and local log sections so exported handoff evidence carries the same daemon/operator snapshot as the live CLI views. Local status now includes structured scheduler/worker poll readiness, queue depth, active leases, capacity, attention reasons, a required/recommended/blocked runbook, and a plan-only daemon service plan so operators can see whether bounded foreground daemon loops are ready to run, what command should happen next, and which foreground entrypoint a future OS supervisor would wrap. Explicitly requesting a wider command execution profile raises the policy action before the command can run; these profiles, local status/service/log views, readiness gates, daemon-readiness hints, handoff summaries, next-action hints, daemon runbooks, daemon service plans, foreground daemon lifecycle metrics, inspection summaries, TUI inspection/next-action/dashboard watch views, local control-plane report/inspection/next-action/review views, local worker leases, and local retry/backoff metadata are local policy/audit boundaries, not a replacement for the future sandboxed runner or production distributed broker.

Local control-plane session verification through `GET /api/sessions/:sessionId/verify?preset=handoff` reuses the same persisted evidence checks as `agent session verify --preset handoff` and TUI `/session verify --preset handoff`. The handoff preset expands required gates from stored session state, reports pass/fail gates, execution profiles, approval-action requirements, and safe follow-up commands from stored audit/session metadata; it does not replay raw tool input, expose prompt/response text, or grant broader execution authority.

Local control-plane session bundles through `GET /api/sessions/:sessionId/bundle?preset=handoff&limit=n` compose the existing safe diff, report, status, timeline, review, result, and verification projections into one local Web package. This endpoint is token-gated like the other local Web session views, keeps prompt/response text and raw tool input out of the bundle, and does not include the CLI export bundle's local status/log snapshots; use `agent session bundle` when an explicit workspace-local export artifact is needed.

Local status also exposes a metadata-only daemon `lifecyclePlan` that classifies the next operator mode (`resolve_attention`, `register_worker`, `run_scheduler_worker`, `worker_standby`, or `idle`) and repeats only safe command strings, readiness flags, attention reasons, and runbook step ids. This plan is derived from persisted local state and does not grant broader execution authority.

Local status and `agent local service` also expose a metadata-only daemon `servicePlan` derived from the same persisted state. It names the platform manager shape, service name, foreground scheduler/worker entrypoints, health/log commands, readiness, blocked steps, and supervision policy as `plan_only`; it does not register, start, stop, uninstall, or mutate an operating-system service.

Diff/result/status/review/bundle summaries also expose a metadata-only diff `inspectionPlan` when completed patch evidence exists. The plan ranks changed file paths using persisted per-file stats, change type, review size, and safe command strings; it does not re-read workspace files, replay pending tool input, or expose prompt/response text.

TUI `/sessions`, `/session diff`, `/session status`, `/session timeline|logs`, `/session review`, and `/session result` are interactive aliases for the same safe session dashboard, diff, status, timeline, review, result, and handoff-summary projections; TUI `/sessions watch` and `/session watch <session-id> [status|inspect|next|review|timeline]` repeat those safe projections for a bounded foreground tick count. These views do not replay raw tool input, prompt text, response text, or secret material beyond bounded patch excerpts in explicit diff/review/result views.
TUI `/approvals`, `/approve`, and `/deny` use the same approval store, scoped capability checks, `tool.approved` / `tool.denied` decision audit records, and room approval messages as the CLI approval path; replay/resume continuation remains explicit through `agent approve --auto-replay`, `--auto-resume`, or `--queue-resume`, and `agent phase2 verify --json --cleanup` now exercises the TUI list/approve/deny path plus a failing `--require-no-pending-approvals` gate while policy approvals remain pending and a passing handoff gate after approvals are resolved as part of the local engineering smoke.
