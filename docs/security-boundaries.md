...
TUI `/session diff`, `/session status`, `/session timeline|logs`, `/session review`, and `/session result` are interactive aliases for the same safe session diff, status, timeline, review, and result projections; they do not replay raw tool input, prompt text, response text, or secret material beyond bounded patch excerpts in explicit diff/review/result views.
