...
- [ ] **Phase 5 cross-machine room collaboration alpha**
  - Source: `docs/agent-rooms.md`, `docs/implementation-roadmap.md`, and the Phase 5 CLI verifier in `src/cli/index.ts`.
  ...
  - Evidence present: `node --test dist\__tests__\remote-room-runner.test.js` passes 87/87, including the registered-agent pull path from remote registration through room invitation listing, acceptance, signed heartbeat, routed task handling through `remote run`, signed delivery ack, and signed room reply evidence. The Phase 5 collector flow is implemented for evidence plan, collection runbook/prepare, collector guide/pack, evidence init/status/template/check/merge, target-dir batch merge, BOM-tolerant JSON reads, missing-evidence summaries, control-plane-host shared room preflight, and remote-target preflight. `node dist\cli\index.js phase5 verify --workspace E:\code\agent --json` returns `status=pass`; `npm test` passes 610/610 in this increment.
