...
| 2. Long-running local coding agent | A single developer can supervise longer changes to real repositories. | Durable plans/goals, robust patch/diff editing, test execution, pause/resume/cancel, approval replay, worker/scheduler loops, operator drilldown, recovery after interruption, retention/compaction, clear diff/result inspection, and daemon-ready `soloclaw` status/log commands. | Partially implemented; worker/scheduler/operator/spec flows exist locally, unified-diff `apply_patch` now has a local failed-test recovery smoke via `agent phase2 verify --json --cleanup`, that verifier also checks persisted `session diff` / `session report` / `session status` / `session timeline` / `session review` / `session result` / `session verify` evidence, end-to-end `agent run` and `agent resume` session-evidence paths, plan/build/goal target-mode evidence, workspace-write/dependency/Git/high-risk-shell approval boundaries, command timeout/duration/profile evidence, a mock agent-loop repair that drives real `run_command` / `apply_patch` / `run_command` tools, and lifecycle pause/resume/cancel audit evidence, `agent run --json --session-result --verify-session`, `agent resume --json --session-result --verify-session`, and `agent plan|build|goal --json --session-result --verify-session` return session/result/verification metadata, `agent session diff <session-id>` exposes persisted patch review, `agent session report <session-id> --json` summarizes session evidence including approvals, timed-out commands, and command execution profiles, `agent session status <session-id>` exposes daemon-ready status snapshots, `agent session timeline|logs <session-id>` exposes ordered safe engineering logs, `agent session review <session-id>` gives an operator review package across checklist, changes, commands, recovery, approvals, and latest timeline, `agent session result <session-id>` gives a human-readable outcome/recovery/change/approval/timeout/profile summary, and `agent session verify <session-id>` provides a non-zero-exit engineering evidence gate including required execution profiles; stronger isolation beyond local policy/audit profiles, daemon service UX, richer generated diff/result inspection, and smooth real-model task driving remain the main gaps. |
...
- Policy and approval flow for workspace tools, secrets, plugins, MCP connection planning, room approvals, and high-risk actions, including synchronous approval replay/resume.
...
4. Policy enforcement and audit.
   ...
   - Current: agent sessions pause on `approval_required`; `approve --auto-replay --auto-resume` executes the approved tool, appends the real tool result, and resumes the session.
   ...
   - Next: move automatic approval replay/resume into a durable worker queue, then add retention-aware export jobs, SIEM sinks, legal hold integration, and admin review workflows.
...
5. Patch/write workflow.
   ...
   - Current: `apply_patch` accepts conservative unified diffs for create/modify/delete inside the workspace, validates protected paths and hunk context before writing, rejects renames for now, and returns per-file before/after hashes.
    ...
    - Current: `agent session diff <session-id> [--json]` reuses session audit to inspect successfully completed `apply_patch` unified diffs and changed paths for review, excluding pending approval requests that have not executed.
    ...
    - Current: `agent session review <session-id> [--json] [--limit n]` packages review state, checklist, changed paths, patch excerpts, commands, recovery, approvals, latest timeline, and follow-up commands for operator handoff.
    - Current: `agent session result <session-id> [--json]` combines session report and diff evidence into an operator-facing outcome, recovery, command, approval, patch, and changed-file summary.
   - Current: `agent session verify <session-id> [--json] [--require-change] [--require-patch] [--require-recovery] [--require-timeout] [--require-execution-profile profile] [--require-approval-action action]` turns persisted engineering evidence into a non-zero-exit verification gate for CI and release smoke use, including timeout, execution-profile, and specific approval-action requirements.
   - Current: `agent run --json --session-result --verify-session` returns the completed session, final answer, session result, verification checks, and follow-up review commands from one end-to-end run invocation.
    ...
    - Current: `agent session report <session-id>` and `agent session result <session-id>` include session-scoped approval counts, pending approval details, command durations, timed-out command counts, and command execution profile counts alongside command, patch, file-change, and recovery evidence.
    - Current: `agent phase2 verify --json --cleanup` exercises disposable sample repos from failing test to patch to passing test, including mock agent-loop repair, resume evidence, plan/build/goal target-mode paths, command timeout/profile/timeline/status/review evidence, and workspace-write/dependency-install/Git-mutation/high-risk-shell policy approval evidence, and reports `phaseClosure: "partial"` with file-change, command-audit, tool-audit, approval, timeout, profile, timeline, status, review, and tool-result evidence.
...
7. TUI and session UX.
   ...
   - Current: `agent session diff <session-id> [--json]` extracts persisted `apply_patch` unified diffs from session audit, maps them to changed paths, and prints patch text for review.
    - Current: `agent session report <session-id> [--json]` summarizes persisted engineering evidence for a session, including messages, tool results, file changes, changed paths, command audit, command durations/timeouts, command execution profiles, approvals, failed tool results, and failed command exits.
    ...
    - Current: `agent session review <session-id> [--json] [--limit n]` gives operators one review package across checklist, changes, commands, recovery, approvals, latest safe timeline, and follow-up commands.
    - Current: `agent session result <session-id> [--json]` summarizes persisted engineering outcomes for a session, including succeeded/failed/paused/cancelled status, whether failed commands recovered, changed files, patch count, command exits, command durations/timeouts, command execution profiles, approvals, and review commands.
    - Current: `agent session verify <session-id> [--json] [--require-change] [--require-patch] [--require-recovery] [--require-timeout] [--require-execution-profile profile] [--require-approval-action action]` reports individual pass/fail evidence checks and exits non-zero when the requested gate is not satisfied.
