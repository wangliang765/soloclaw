...
| 2. Long-running local coding agent | A single developer can supervise longer changes to real repositories. | Durable plans/goals, robust patch/diff editing, test execution, pause/resume/cancel, approval replay, worker/scheduler loops, operator drilldown, recovery after interruption, retention/compaction, clear diff/result inspection, and daemon-ready `soloclaw` status/log commands. | Partially implemented; worker/scheduler/operator/spec flows exist locally, unified-diff `apply_patch` now has a local failed-test recovery smoke via `agent phase2 verify --json --cleanup`, that verifier also checks persisted `sessions` / `session diff` / `session report` / `session status` / `session timeline` / `session review` / `session result` / `session verify` / `session bundle` evidence, end-to-end `agent run` and `agent resume` session-evidence paths, plan/build/goal target-mode evidence, workspace-write/dependency/Git/high-risk-shell approval boundaries, command timeout/duration/profile/diff-stat/file-summary/session-list/local-agent-status/local-agent-logs evidence, a mock agent-loop repair that drives real `run_command` / `apply_patch` / `run_command` tools, and lifecycle pause/resume/cancel audit evidence, `agent run --json --session-result --verify-session`, `agent resume --json --session-result --verify-session`, and `agent plan|build|goal --json --session-result --verify-session` return session/result/verification metadata, `agent sessions --json --limit n` exposes a local session dashboard with outcome, pending approvals, command/change counts, latest safe timeline, and review commands, `agent local status --json` / `soloclaw agent status --json` expose a local agent status snapshot across sessions, workers, assignments, and approvals, `agent local logs --json` / `soloclaw agent logs --json` expose merged safe execution logs, `agent session diff <session-id>` exposes persisted patch review plus per-file additions/deletions, change type, patch count, review size, and review hint, `agent session report <session-id> --json` summarizes session evidence including approvals, timed-out commands, command execution profiles, diff stats, and file summaries, `agent session status <session-id>` exposes daemon-ready status snapshots, `agent session timeline|logs <session-id>` exposes ordered safe engineering logs, `agent session review <session-id>` gives an operator review package across checklist, changes, diff stats, file summaries, commands, recovery, approvals, and latest timeline, `agent session result <session-id>` gives a human-readable outcome/recovery/change/approval/timeout/profile/diff-stat/file-summary summary, `agent session verify <session-id>` provides a non-zero-exit engineering evidence gate including required execution profiles and diff stats, and `agent session bundle <session-id>` exports diff/report/status/timeline/review/result/verification evidence in one workspace-local JSON package; stronger isolation beyond local policy/audit profiles, daemon service lifecycle UX, richer generated diff/result inspection, and smooth real-model task driving remain the main gaps. |
...
5. Patch/write workflow.
   ...
   - Current: `apply_patch` accepts conservative unified diffs for create/modify/delete inside the workspace, validates protected paths and hunk context before writing, rejects renames for now, and returns per-file before/after hashes.
    ...
    - Current: `agent session diff <session-id> [--json]` reuses session audit to inspect successfully completed `apply_patch` unified diffs, changed paths, per-file additions/deletions, change type, patch count, review size, and review hint for review, excluding pending approval requests that have not executed.
    - Current: `agent session status <session-id> [--json] [--limit n]` returns a daemon-ready snapshot of outcome, command counts, pending approvals, execution profiles, and latest safe timeline items.
    ...
    - Current: `agent local status [--json] [--limit n]` and `soloclaw agent status [--json] [--limit n]` aggregate recent sessions, pending approvals, worker registrations, assignment queue/load state, and next commands into one daemon-ready local status view.
    - Current: `agent local logs [--json] [--limit n]` and `soloclaw agent logs [--json] [--limit n]` merge safe audit, file-change, approval, and approval-decision items across recent local sessions for foreground supervision and future TUI/Web log panels.
    - Current: `agent session timeline|logs <session-id> [--json] [--limit n]` combines audit events, file changes, approval requests, and approval decisions into an ordered engineering log with safe metadata.
    - Current: `agent session review <session-id> [--json] [--limit n]` packages review state, checklist, changed paths, diff stats, patch excerpts, commands, recovery, approvals, latest timeline, and follow-up commands for operator handoff.
    - Current: `agent session result <session-id> [--json]` combines session report and diff evidence into an operator-facing outcome, recovery, command, approval, patch, diff-stat, file-summary, and changed-file summary.
   - Current: `agent session verify <session-id> [--json] [--require-change] [--require-patch] [--require-recovery] [--require-timeout] [--require-diff-stat] [--require-execution-profile profile] [--require-approval-action action]` turns persisted engineering evidence into a non-zero-exit verification gate for CI and release smoke use, including timeout, diff-stat, execution-profile, and specific approval-action requirements.
   - Current: `agent run --json --session-result --verify-session` returns the completed session, final answer, session result, verification checks, and follow-up review commands from one end-to-end run invocation.
    ...
    - Current: `agent session report <session-id>` and `agent session result <session-id>` include session-scoped approval counts, pending approval details, command durations, timed-out command counts, command execution profile counts, diff stats, and file summaries alongside command, patch, file-change, and recovery evidence.
    - Current: `agent phase2 verify --json --cleanup` exercises disposable sample repos from failing test to patch to passing test, including mock agent-loop repair, resume evidence, plan/build/goal target-mode paths, command timeout/profile/diff-stat/file-summary/timeline/status/session-list/local-agent-status/local-agent-logs/review/bundle evidence, and workspace-write/dependency-install/Git-mutation/high-risk-shell policy approval evidence, and reports `phaseClosure: "partial"` with file-change, command-audit, tool-audit, approval, timeout, profile, diff-stat, file-summary, timeline, status, session-list, local-agent status/logs, review, bundle, and tool-result evidence.
...
6. Real model providers.
   ...
   - Current: `agent models usage` summarizes local `model.called` audit events by provider/model, including calls, failures, provider-reported token usage, duration, and optional caller-supplied cost estimates.
...
7. TUI and session UX.
   ...
   - Current: `agent session diff <session-id> [--json]` extracts persisted `apply_patch` unified diffs from session audit, maps them to changed paths, reports per-file additions/deletions, change type, patch count, review size, and review hint, and prints patch text for review.
    ...
    - Current: `agent local status [--json] [--limit n]` / `soloclaw agent status [--json] [--limit n]` gives everyday operators a single local status surface across sessions, workers, assignments, approvals, queue load, and follow-up commands.
    - Current: `agent local logs [--json] [--limit n]` / `soloclaw agent logs [--json] [--limit n]` gives everyday operators a merged safe log across local session audit, file-change, and approval evidence.
    - Current: `agent session report <session-id> [--json]` summarizes persisted engineering evidence for a session, including messages, tool results, file changes, changed paths, command audit, command durations/timeouts, command execution profiles, diff stats, file summaries, approvals, failed tool results, and failed command exits.
    - Current: `agent session status <session-id> [--json] [--limit n]` gives daemon/TUI/Web callers a compact status document with latest timeline entries and review commands.
    - Current: `agent session timeline|logs <session-id> [--json] [--limit n]` gives operators an ordered safe log across audit events, file changes, approvals, and approval decisions.
    - Current: `agent session review <session-id> [--json] [--limit n]` gives operators one review package across checklist, changes, diff stats, file summaries, commands, recovery, approvals, latest safe timeline, and follow-up commands.
    - Current: `agent session result <session-id> [--json]` summarizes persisted engineering outcomes for a session, including succeeded/failed/paused/cancelled status, whether failed commands recovered, changed files, patch count, diff stats, file summaries, command exits, command durations/timeouts, command execution profiles, approvals, and review commands.
    - Current: `agent session verify <session-id> [--json] [--require-change] [--require-patch] [--require-recovery] [--require-timeout] [--require-diff-stat] [--require-execution-profile profile] [--require-approval-action action]` reports individual pass/fail evidence checks and exits non-zero when the requested gate is not satisfied.
   ...
   - Current: `agent resume <session-id> --json --session-result --verify-session` gives the same outcome and verification package after a paused session is continued.
