...
5. Patch/write workflow.
   ...
   - Current: `apply_patch` accepts conservative unified diffs for create/modify/delete inside the workspace, validates protected paths and hunk context before writing, rejects renames for now, and returns per-file before/after hashes.
    ...
    - Current: `agent session review <session-id> [--json] [--limit n]` and TUI `/session review <session-id> [--json] [--limit n]` package review state, checklist, changed paths, diff stats, file summaries, review profile, patch excerpts, commands, recovery, approvals, latest timeline, operator next actions, and follow-up commands for operator handoff.
    - Current: `agent session result <session-id> [--json]` and TUI `/session result <session-id> [--json]` combine session report and diff evidence into an operator-facing outcome, recovery, command, approval, patch, diff-stat, file-summary, review-profile, changed-file, and next-action summary.
...
7. TUI and session UX.
   ...
   - Current: `agent session diff <session-id> [--json]` and TUI `/session diff <session-id> [--json]` extract persisted `apply_patch` unified diffs from session audit, map them to changed paths, report per-file additions/deletions, change type, patch count, review size, review hint, and aggregate review profile, and print patch text for review.
    ...
    - Current: `agent session review <session-id> [--json] [--limit n]` and TUI `/session review <session-id> [--json] [--limit n]` give operators one review package across checklist, changes, diff stats, file summaries, commands, recovery, approvals, latest safe timeline, operator next actions, and follow-up commands.
    - Current: `agent session result <session-id> [--json]` and TUI `/session result <session-id> [--json]` summarize persisted engineering outcomes for a session, including succeeded/failed/paused/cancelled status, whether failed commands recovered, changed files, patch count, diff stats, file summaries, command exits, command durations/timeouts, command execution profiles, approvals, metadata-only model usage, inspection issues/focus paths, next actions, and review commands.
